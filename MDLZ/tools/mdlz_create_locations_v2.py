...
def create_store_locations(pc, mdlz_stores, mdlz_dc_locations, on_progress=None):
    ...
    for _, row in mdlz_stores.iterrows():
        ...
        if missing:
            skipped += 1
            print(f"  SKIP: row missing {missing}")
            processed += 1
            if on_progress:
                on_progress(processed, total, {"status": "skipped", "store_name": str(row.get("name", "unknown")), "reason": f"missing {missing}"})
            continue
        ...
        if name in seen_names_in_file:
            print(f"  SKIP (duplicate in input): {name} already appears earlier in this file")
            processed += 1
            result = {"ecc": str(row.get("ECC", "")), "store_name": name, "location_id": None, "status": "skipped", "reason": "duplicate name in input file"}
        ...
        if name in existing_names:
            ...
            processed += 1
            result = {"ecc": ecc, "store_name": name, "location_id": None, "status": "skipped", "reason": "already exists"}
            results.append(result)
            if on_progress:
                on_progress(processed, total, result)
            continue
...
def create_trailer_locations(pc, mdlz_trailers, mdlz_dc_locations, on_progress=None):
    ...
    for _, row in mdlz_trailers.iterrows():
        ...
        if missing:
            ...
            processed += 1
            if on_progress:
                on_progress(processed, total, {"status": "skipped", "location_name": str(row.get("name", "unknown")), "reason": f"missing {missing}"})
            continue
        ...
        if name in seen_names_in_file:
            ...
            result = {"unit_num": zone_name, "location_name": name, "location_id": None, "zone_id": None, "status": "skipped", "reason": "duplicate name in input file"}
            results.append(result)
            if on_progress:
                on_progress(processed, total, result)
            continue
        ...
        trailer_make = str(row["trailerMake"]).strip()
        associate_dc = str(row["ASSOCIATE_DC"]).strip()
        ...
        if name in existing_names:
            print(f"  SKIP (duplicate): {name} already exists on platform")
            processed += 1
            result = {"unit_num": zone_name, "location_name": name, "location_id": None, "zone_id": None, "status": "skipped", "reason": "already exists"}
            results.append(result)
            if on_progress:
                on_progress(processed, total, result)
            continue
